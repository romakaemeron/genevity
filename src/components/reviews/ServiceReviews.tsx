...
/**
 * Patient reviews for one service, as a carousel (TZ #10 §4).
 *
 * The review text is rendered into the HTML on the server — the spec is
 * explicit that it must be in the code, not loaded by script — and only the
 * horizontal scrolling is client-side. Shares the card design and the
 * snap-scroll mechanics with `DoctorReviews` so the two blocks read as the
 * same component in different places.
 */
...
import type { ServiceReview } from "@/lib/db/types";
...
const BY_DOCTOR: Record<string, string> = {
  ua: "Лікар, який надав послугу",
  ru: "Врач, который предоставил услугу",
  en: "Treated by",
};
    ...
    <div className="flex items-center gap-0.5 text-main" aria-label={`${n} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          ...
          stroke={i <= n ? "currentColor" : "#d1c7bb"}
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      <div className="flex flex-col gap-2 pt-3 border-t border-black/[0.06]">
        ...
        {doctorName && (
          <p className="body-s text-black-50">
            {BY_DOCTOR[locale] ?? BY_DOCTOR.ua}:{" "}
            {doctorSlug ? (
              <Link href={`/doctors/${doctorSlug}`} className="text-main hover:underline">
                {doctorName}
              </Link>
            ) : (
              <span className="text-black-70">{doctorName}</span>
            )}
          </p>
        )}
      </div>
